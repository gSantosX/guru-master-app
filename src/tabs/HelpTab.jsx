  ...
  const tutorialVideos = [
    {
      ...
      duration: '12:45',
      videoId: null // Placeholder for future video
    },
    {
      id: 'connect-api',
            ...
            <div className="space-y-4">
              {tutorialVideos.map((video, idx) => (
                <motion.div
                  ...
                  className={`glass-card group overflow-hidden border border-white/5 transition-all ${video.videoId ? 'hover:border-neon-cyan/30 cursor-pointer' : 'opacity-70'}`}
                  onClick={() => video.videoId && setActiveVideo(video.videoId)}
                >
                  <div className="flex flex-col md:flex-row">
                    ...
                    <div className="w-full md:w-48 h-32 bg-dark/80 relative flex items-center justify-center overflow-hidden border-b md:border-b-0 md:border-r border-white/5">
                      {video.videoId ? (
                        <>
                          ...
                          />
                          <PlayCircle className="w-10 h-10 text-white shadow-2xl group-hover:text-neon-cyan group-hover:scale-110 transition-all duration-500 z-10 drop-shadow-lg" />
                        </>
                      ) : (
                      ...
                      <div className="absolute bottom-2 right-2 px-2 py-0.5 rounded bg-black/80 text-[9px] text-gray-400 font-mono font-bold border border-white/10 z-10">
                        {video.duration}
                      </div>
                    </div>
                    ...
                    <div className="flex-1 p-6 flex flex-col justify-center">
                      ...
                      <button className={`w-fit flex items-center gap-2 text-[10px] font-black uppercase tracking-widest ${video.videoId ? 'text-neon-cyan hover:text-white' : 'text-gray-600'} transition-colors`}>
        ...
        {activeVideo && (
          ...
          >
            <motion.div
              ...
              className="relative w-full max-w-5xl aspect-video bg-[#0a0a0a] border border-white/10 rounded-2xl overflow-hidden shadow-2xl flex flex-col"
              onClick={e => e.stopPropagation()}
            >
              {/* Header inside modal */}
              <div className="absolute top-0 inset-x-0 p-4 bg-gradient-to-b from-black/80 to-transparent flex justify-end items-center z-10 pointer-events-none">
                <button
                  onClick={() => setActiveVideo(null)}
                  className="p-2 rounded-xl bg-black/50 hover:bg-neon-cyan/20 border border-white/10 hover:border-neon-cyan/50 text-white transition-all pointer-events-auto"
                >
                  <X className="w-5 h-5" />
                </button>
              </div>
