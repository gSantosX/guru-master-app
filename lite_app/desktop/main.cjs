...
function createWindow() {
  ...
  // Tenta carregar o build de produção primeiro se estiver na pasta lite_app
...
function startBackend() {
  ...
  // Prioridade 1: Python Portátil do instalador Zero-Config
  let venvPath = path.join(__dirname, '../bin/python/python.exe');
  
  // Prioridade 2: Venv tradicional de desenvolvimento
  if (!fs.existsSync(venvPath)) {
    venvPath = path.join(__dirname, '../backend/venv/Scripts/python.exe');
  }

  // Prioridade 3: Python do sistema (último recurso)
  if (!fs.existsSync(venvPath)) {
    venvPath = 'python';
  }
